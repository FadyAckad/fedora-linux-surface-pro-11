...
#   the support RPM's %posttrans (installed next, as one dnf transaction would) regenerates the menu.
...
# kernel-install exits non-zero in a chroot (the layout resolves to "other" and 90-loaderentry.install quits, see
# CLAUDE.md) although 20-grub.install has written the entry; the RPM's %posttrans ignores the status the same way.
...
log "--- rpm -U $(basename "$SRPM" .rpm) with scriptlets (the support RPM of the same transaction)"
...
log "menu regenerated by the support RPM"
