...
# unit's calibration; the framework does not initialise the sensors without it) and Surface's calibration overrides.
...
# A cancelled UAC prompt makes Start-Process fail without a process object; that must not pass as exit 0.
powershell.exe -NoProfile -Command "try { \$p = Start-Process -FilePath powershell.exe -Verb RunAs -Wait -PassThru -WindowStyle Hidden -ErrorAction Stop -ArgumentList '-NoProfile','-ExecutionPolicy','Bypass','-File','$PS1_WIN' } catch { Write-Host \$_; exit 99 }; if (-not \$p) { exit 98 }; exit \$p.ExitCode" 2>&1 | tr -d '\r' | sed 's/^/  powershell: /' >&2
rc=${PIPESTATUS[0]}
...
# The framework's bookkeeping files sit one level up on Windows; the reference setup ships them next to the
# registry files, and hexagonrpcd serves only that directory.
...
log "registry: $(find "$REG" -type f | wc -l) files (reference setup: 321), $(stat -c %s "$REG/sns_reg_config") bytes sns_reg_config, $(stat -c %s "$REG/sns_secure_database.bin") bytes sns_secure_database.bin"
