...
- The kernel is Fedora's own `kernel` source RPM for `FEDORA_RELEASE` (`KERNEL_SRPM`, sha256 pinned in `sp11.conf`;
  ...
  revisions 3 and 4 (configuration additions only) on 2026-09-24.
...
- Configuration: Fedora's (`CONFIG_LSM="lockdown,yama,integrity,selinux,bpf,landlock,ipe"`, SELinux default,
  ...
  `SENSORS_SURFACE_FAN`, `BATTERY_SURFACE` — patch 0059 keeps it from binding on the SP11), battmgr and UCSI over
...
- Patch 0046 (OLED link-rate quirk) matches `DMI_PRODUCT_NAME` exactly against "Microsoft Surface Pro, 11th
...
- Rebase to a new Fedora kernel (7.2.6 and later): set `KERNEL_FEDORA_VERSION`, `KERNEL_FEDORA_RELEASE` and the SRPM
  ...
  (never rewrite a pushed one: sp11.conf pins its commits), set `KERNEL_PATCH_BASE_COMMIT` and
  `KERNEL_PATCH_COMMIT`, and bump `KERNEL_SP11_REV`. Known at 7.2.6/7.2.7: `spi_geni_init`'s runtime PM (0053),
  `q6apm.c` graph start/stop (0056), an off-by-one in `soundwire/qcom.c` next to the SP11 change (0055); 0045 (QMP
